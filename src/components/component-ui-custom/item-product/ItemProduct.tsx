...
function ItemProduct(props: IProps) {
  ...
  return (
    <Link href={`/san-pham/${item?.slug?.trim()}`} className="item-slider-product rounded-2xl overflow-hidden  cursor-pointer relative max-md:mb-[2rem]">
      <div
        style={{
      ...
      <div className="relative z-2  -mt-[5.1rem] z-9 left-0 right-0 w-full box-slide max-md:h-[35.5rem] max-md:-mt-[15.1rem]">
        ...
        <div className="relative p-[1rem] rounded-2xl bg-[#FFF] box-slide max-md:p-[2.13rem] max-md:rounded-[3.2rem]">
          <div className={`${keySlide !== "flash-sale" ? "h-[2.4rem] max-md:h-[10.45334rem]" : ""} mb-[0.5rem]`}>
          ...
          {keySlide === "flash-sale" ?
            <div className="md:flex justify-between">
              <div className="text-[2.66667rem] md:text-[0.875rem] text-[#6A6A6A] font-bold md:font-extrabold">
                Tiết kiệm được 
                {item?.salePrice && item?.salePrice !== 0 && (<span className="text-[#F58F5D] pl-2">{formatCurrencyVND(item?.salePrice.toString())}</span>)}
              </div>
          ...
          <div>
            <div className={`hidden max-md:flex price-product-slide justify-between items-center rounded-[10.66667rem] py-[1.6rem] px-[3.2rem] mt-[2rem] ${keySlide === "flash-sale" ? "bg-[#55D5D2]" : "border-[1px] border-[#55D5D2]"}`}>
              ...
              <div className={`arrow-peoduct-slide p-[0.5rem] ${keySlide === "flash-sale" ? "text-[#fff]" : "text-[#55D5D2]"}`}>
                <ArrowTopRight />
              </div>
            </div>
          </div>

...
const Contact = () => {
  ...
  const getListContact = useSWR(`${baseUrl}${paramApi.urlContact}`, () =>
    fetchDataRest(paramApi.method, paramApi.urlContact).then((res: any) => setDataContact(res[0]))
  );
  const getListBlog = useSWR(`${baseUrl}${paramApi.urlBlog}`, () =>
    fetchDataRest(paramApi.method, paramApi.urlBlog).then((res: any) => setDataBlog(res))
  );
  useEffect(() => {
    getListContact
    getListBlog
  ...
  return (
    <div className="md:max-w-[83.75rem] py-24 md:py-12 px-[5rem] md:px-0 m-auto">
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="flex justify-end mb-4 md:mb-2">
          ...
          <button type="submit" className="text-white p-6 md:p-[0.65rem] text-[3rem] md:text-[1.25rem] bg-[#81C8C2]">
            Post Comments
          </button>
        </form>
      </FormProvider>
      ...
      <div className="flex justify-between flex-wrap">
        <div
          ...
          dangerouslySetInnerHTML={{ __html: `${dataContact ? dataContact?.content?.rendered : ''}` }}
        ></div>
        <div className='w-full md:w-1/3'>
          <div>
            <h4 className='font-semibold text-[4.5rem] md:text-[2rem]'>Recent Posts</h4>
            <div className="flex flex-wrap">
              {
                dataBlog?.map((data: BlogItemType, index: number) => (
                  <Link href="#" className="text-[4.5rem] md:text-[1.75rem] w-full py-6 md:py-2">
                    {data?.title}
                  </Link>
                ))
              }
          ...
          <div className="mt-12 md:mt-8">
            <h4 className='font-semibold text-[4.5rem] md:text-[2rem]'>Recent Comments</h4>
            <div className="flex flex-wrap">
